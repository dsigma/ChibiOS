 ...
 * @isr
 */
CH_IRQ_HANDLER(STM32_UART8_HANDLER) {
...
void sd_lld_start(SerialDriver *sdp, const SerialConfig *config) {
  ...
  if (sdp->state == SD_STOP) {
    ...
    if (&SD7 == sdp) {
      ...
      nvicEnableVector(STM32_UART7_NUMBER,
                       CORTEX_PRIORITY_MASK(STM32_SERIAL_UART7_PRIORITY));
    }
#endif
#if STM32_SERIAL_USE_UART8
    if (&SD8 == sdp) {
      rccEnableUART8(FALSE);
      nvicEnableVector(STM32_UART8_NUMBER,
                       CORTEX_PRIORITY_MASK(STM32_SERIAL_UART8_PRIORITY));
    }
#endif
  }
